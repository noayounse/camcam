...
public class BoundingBox {
	  ...
	  public ArrayList<PVector> makePaddedAdjustedBoundingBoxForAspect(PVector normalIn, ArrayList<PVector> pointsIn, float topAddition, float rightAddition, float bottomAddition, float leftAddition, float frontAddition, float rearAddition, float aspectIn) {
	    ...
	    if (initialWidth / initialHeight > aspectIn) {
	      // increase height
	      float heightIncrease = (float) (.5 * (initialWidth / (aspectIn) - initialHeight));
	      ...
	      // increase width
	      float widthIncrease = (float) (.5 * (aspectIn * initialHeight - initialWidth));
	  ...
	  public float distanceFromPointToPlane (PVector normalIn, PVector planePoint, PVector questionPoint) {
	    float distanceDifference = (float)((normalIn.x * (planePoint.x - questionPoint.x) + normalIn.y * (planePoint.y - questionPoint.y) + normalIn.z *(planePoint.z - questionPoint.z)) / Math.sqrt(normalIn.x * normalIn.x + normalIn.y * normalIn.y + normalIn.z * normalIn.z));

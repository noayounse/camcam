public class CamCam {
  ...
  private float heightScale = 1f; // in case a manual override of scale is needed...
  ...
  private void setupTweens () {
    ...
    makeScreenLines(leftFrustum.value(), rightFrustum.value(), parent.height, 0);
  } // end setupTweens
  ...
  public void makeFrustum(float leftIn, float rightIn, float frameOfViewIn, float depthIn) {
    ...
    parent.frustum(-left * aspect * heightScale, right * aspect * heightScale, -(1f / 2) * heightScale, (1f / 2) * heightScale, .5f / (float)(Math.atan(fovy / 2)), depthIn);
  } // end makeFrustum

  public void makeScreenLines(float leftIn, float rightIn, float topIn, float bottomIn) {
  ...
  public float adjustForNearestRotation(float angleIn, float currentAngle) {
    ...
    if (diffOriginal <= diffLower && diffOriginal <= diffHigher) angleIn = angleIn;
    else if (diffLower < diffOriginal) angleIn = angleLower;
    else angleIn = angleHigher;
    return angleIn;
    ...
    if (distFromTargetPlane > distFromCamPlane) {
      difference = distFromCamPlane - getDistance();
  ...
  public boolean pointInView(PVector p, float newLow, float newHigh) {
    ...
    if (pointOutOfBounds(p)) return false;
    PVector in2dSpace = new PVector(parent.screenX(p.x, p.y, p.z), parent.screenY(p.x, p.y, p.z));
    if (in2dSpace.x >= leftFrustum.value() && in2dSpace.x <= rightFrustum.value() && in2dSpace.y >= newLow && in2dSpace.y <= newHigh) return true;
    return false;
  } // end pointInView
  ...
  public boolean rectInView(ArrayList<PVector> ptsIn, float newLow, float newHigh) {
    for (PVector p : ptsIn)
    ...
    if (screenLines.length >= 1) {
      PVector screenCorner = new PVector((float)screenLines[0].getX1(), (float)screenLines[0].getY1());
      if (isInsidePolygon(screenCorner, corners)) return true;
    }
    return false;
  } // end rectInView
  ...
  public boolean isInsidePolygon(PVector pos, ArrayList<PVector> verticesIn) {
    ...
    for (int i = 0; i < temp.length; i++) temp[i] = verticesIn.get(i);
  ...
  public boolean isInsidePolygon(PVector pos, PVector[] verticesIn) {
    ...
    for (i=0; i<sides; i++) {
      if ((verticesIn[i].y < pos.y && verticesIn[j].y >= pos.y || verticesIn[j].y < pos.y && verticesIn[i].y >= pos.y) && (verticesIn[i].x <= pos.x || verticesIn[j].x <= pos.x)) {
        oddNodes^=(verticesIn[i].x + (pos.y-verticesIn[i].y)/(verticesIn[j].y - verticesIn[i].y)*(verticesIn[j].x-verticesIn[i].x)<pos.x);
      }
      j=i;
    }
    return oddNodes;
  ...
  // makeRectCorners will take in the 3d points, adjust them, then return the four 2d screen points
  public ArrayList<PVector> makeRect2dCorners(ArrayList<PVector> ptsIn) {
    ...
    if (outOfBoundsPoints == 4) {
      ...
      for (PVector p : ptsIn) newNew3DCorners.add(p.get());
      for (int i = 0; i < obIndices.length; i++) {
        ...
        if (obIndices[i] == 1) {
          ...
          int otherIndex = nextIndex + 1 < ptsIn.size() ? nextIndex + 1 : 0;
          if (obIndices[nextIndex] == 0 && obIndices[previousIndex] == 0) {
            // if both previous and next are valid, use the one that is furthest from the camera
            float distNext = getDistanceFromCameraPlane(ptsIn.get(nextIndex).get());
            float distPrev = getDistanceFromCameraPlane(ptsIn.get(previousIndex).get());
            if (distNext > distPrev) newPoint = new3dPoint(ptsIn.get(nextIndex).get(), newPoint);
            else newPoint = new3dPoint(ptsIn.get(previousIndex).get(), newPoint);
          }
          else if (obIndices[nextIndex] == 0) newPoint = new3dPoint(ptsIn.get(nextIndex).get(), newPoint);
          else if (obIndices[previousIndex] == 0) newPoint = new3dPoint(ptsIn.get(previousIndex).get(), newPoint);
          else if (obIndices[otherIndex] == 0) newPoint = new3dPoint(ptsIn.get(otherIndex).get(), newPoint);
        }
    ...
    else new2dCorners = make2dCorners(ptsIn);
    return new2dCorners;
  } // end makeRect2dCorners

    private ArrayList<PVector> make2dCorners(ArrayList<PVector> ptsIn) {
    ArrayList<PVector> new2DCorners = new ArrayList<PVector>(); 
    for (PVector p : ptsIn) new2DCorners.add(new PVector(screenX(p.x, p.y, p.z), screenY(p.x, p.y, p.z)));
    return new2DCorners;
  } // end make2dCorners
